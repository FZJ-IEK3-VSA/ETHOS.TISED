...
class SolarModel:
    ...
    def handle_missing_hourly_data(self):
        ...
        self.hourly_database_ghi, self.minutal_database_ghi = load_data_for_zone(
            self.zone
        )
    ...
    def get_clear_sky_irradiance_hr(self):
        ...
        self.hourly_irrad_cs_ghi = (
            self.hourly_cs[["ghi_extra"]]
            .reset_index(drop=True)
            .to_numpy()
            .reshape(self.noOfHrs)
        )
    ...
    def get_clear_sky_irradiance_min(self):
        ...
        self.min_irrad_cs_ghi = (
            self.minutal_cs[["ghi_extra"]]
            .reset_index(drop=True)
            .to_numpy()
            .reshape(self.noOfMins)
        )
    ...
    def train_knn(self, hourly_database_ghi):
        ...
        self.neigh_ghi.fit(
            self.hourly_database_ghi[:, [1, 2, 3, 4, 5]], self.hourly_database_ghi[:, 0]
        )
    ...
    def get_solar_angles(self):
        ...
        self.omega_deg = pvlib.solarposition.hour_angle(
            date_range, longitude=self.lon, equation_of_time=self.EOT
        )
    ...
    def compute_clear_sky_irradiance(self):
        ...
        data_A = self.hourly_cs["ghi_extra"].values.reshape(
            (self.days, self.points_per_day)
        )
        # data_A
    ...
    def compute_variability_indices(self):
        ...
        for i in range(data_A1.shape[1]):
            ...
            a_values[i] = np.power(
                np.power(daily_differences_a_values, 2) + 1, 1 / 2
            ).sum()
        ...
        for i in range(data_B1.shape[1]):
            ...
            # Compute the value of a for the current column
            b_values[i] = np.power(
                np.power(daily_differences_b_values, 2) + 1, 1 / 2
            ).sum()
        ...
        for i in range(data_C1.shape[1]):
            ...
            # Compute the value of a for the current column
            c_values[i] = np.power(
                np.power(daily_differences_c_values, 2) + 1, 1 / 2
            ).sum()
    ...
    def compute_morning_fraction(self):
        ...
        hourly_irrad_m_after_noon_ghi_2d = np.copy(
            self.hourly_irrad_m_ghi_2d
        )  # copy of measured irradiance array
        hourly_irrad_m_after_noon_ghi_2d[after_noon_mask_2d] = (
            0  # setting afternoon values to 0 -
        )

        Fm_ghi = hourly_irrad_m_after_noon_ghi_2d.sum(
            1
        ) / self.hourly_irrad_m_ghi_2d.sum(
            1
    ...
    def compute_normalized_ic_cdf(self):
        ...
        for day in range(self.days):
            daily_data = data[
                day * self.points_per_day : (day + 1) * self.points_per_day
            ]
    ...
    def predict_knn(self, neigh_ghi):
        ...
        similar_days_ghi = self.neigh_ghi.predict(
            calculated_indicators_ghi
        )  # predicting the class labels for similar days from calculated indices
    ...
    def upscale_to_minute_resolution(self, similar_days_ghi):
        ...
        for i in range(0, self.days):  # iterating for the year across each day
            ups_day_ghi = self.minutal_database_ghi[
                np.where(self.minutal_database_ghi[:, 0] == similar_days_ghi[i])
            ]  # gets the data from minutal dataset for each similar day -- in other words, get 1000 rows for each similar day
            indices = np.around(
                1000
                * np.arange(1, np.sum(self.pos_alpha_deg_mask_min_2d[i]) + 1)
                / np.sum(self.pos_alpha_deg_mask_min_2d[i])
            ...
            syn_day[self.pos_alpha_deg_mask_min_2d[i]] = ups_day_ghi[
                indices.astype(int) - 1
            ][
                :, 1:3
    ...
    def generate_synthetic_data(self):
        ...
        df_synthetic_non_optimized = (self.min_syn_kt * self.min_irrad_cs_ghi).reshape(
            self.days, self.minutes_per_day
        )
        # A = pd.DataFrame(A)
