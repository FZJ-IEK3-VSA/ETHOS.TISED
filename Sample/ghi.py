...
hourly_irrad_m = np.genfromtxt(
    r"hourly_irrad_m_modified_new.csv", delimiter=","
)  # importing hourly timeseries- should be 8760 items for one year, the columns are day (1-365), hour (1-24), GHI (W/m2)
hourly_irrad_m
...
)  # training the KNN classifier using the inputs as columns 1, 2, 3, 4, and 5 and the labels from column 0 as the target variable for classification
...
Kt = (
    hourly_irrad_m_ghi_2d.mean(1) / hourly_irrad_cs_ghi_2d.mean(1)
...
neg_alpha_deg_mask_2d_k = np.delete(
    neg_alpha_deg_mask_2d, 0, 1
)  # deletes the first column from the 2D matrix


hourly_irrad_cs_ghi_2d_k = np.delete(
    hourly_irrad_cs_ghi_2d, 0, 1
)  # deletes the first column from the clear sky irradiance array
hourly_irrad_cs_ghi_2d_k[neg_alpha_deg_mask_2d_k] = (
    0  # masks using the newly transformed arrays
)

hourly_irrad_cs_ghi_2d_k_less1 = np.delete(
    hourly_irrad_cs_ghi_2d, 23, 1
)  # creates a new clear sky irradiance array with one less interval (n-1)
hourly_irrad_cs_ghi_2d_k_less1[neg_alpha_deg_mask_2d_k] = (
    0  # masks using the newly transformed arrays
)

hourly_irrad_m_ghi_2d_k = np.delete(
    hourly_irrad_m_ghi_2d, 0, 1
)  # deletes the first column from the measured irradiance array
hourly_irrad_m_ghi_2d_k[neg_alpha_deg_mask_2d_k] = (
    0  # masks using the newly transformed arrays
)

hourly_irrad_m_ghi_2d_k_less1 = np.delete(
    hourly_irrad_m_ghi_2d, 23, 1
)  # creates a new measured irradiance array with one less interval (n-1)
hourly_irrad_m_ghi_2d_k_less1[neg_alpha_deg_mask_2d_k] = (
    0  # masks using the newly transformed arrays
)

numerator_ghi = (
    np.power(
        np.power(hourly_irrad_m_ghi_2d_k - hourly_irrad_m_ghi_2d_k_less1, 2) + 1, 1 / 2
    )
).sum(1)  # numerator of VI (see Larraneta et al., 2017)
denominator_ghi = (
    np.power(
        np.power(hourly_irrad_cs_ghi_2d_k - hourly_irrad_cs_ghi_2d_k_less1, 2) + 1,
        1 / 2,
    )
).sum(1)  # denominator of VI (see Larraneta et al., 2017)
